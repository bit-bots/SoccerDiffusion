...
class StepEncoding(nn.Module):
    def __init__(self, d_model):
        ...
        self.div_term = torch.exp(torch.arange(0, d_model, 2, requires_grad=False).float() * (-np.log(10000.0) / d_model)).to(device)
...
model = TrajectoryTransformerModel(num_joints=trajectory_dim, hidden_dim=hidden_dim, num_layers=num_layers,
                                   num_heads=num_heads, max_seq_len=sequence_length).to(device)
